...
class MMOKD(nn.Module):
    def __init__(self, num_classes=2):
        super(MMOKD, self).__init__()

        self.eeg_model_name = 'MCGRAM'
        self.pic_model_name = 'EfficientNet'
        self.MCGRAM = gcn()

        self.efficientNet = efficient()

        self.pic_fc = nn.Linear(1280, 64)#for EfficientNet in fusion

        self.eeg_fc = nn.Linear(256, 64)#aligment for MCGRAM in fusion
    ...
    def get_eeg_fea(self,data):
        if self.eeg_model_name == 'MCGRAM':
            eeg_fea = self.MCGRAM(data)#(batchsize,256)
            eeg_x = self.fc_eeg(eeg_fea)

        eeg_classes = eeg_x
        return eeg_classes

    def get_pic_fea(self,data):
        pic_data, batch_size = data.pic, len(data.dataset_idx)
        pic_data = pic_data.view(batch_size, 3, 224, 224)
        if self.pic_model_name == 'EfficientNet':
            pic_fea = self.efficientNet(pic_data)
            pic_x = self.fc_pic_pic(pic_fea)#for efficientnet
