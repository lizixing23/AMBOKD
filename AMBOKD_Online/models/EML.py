...
class EML(nn.Module):
    ...
    def get_fusion_fea(self,eeg_fea,pic_fea):

        fusion_fea = torch.cat([eeg_fea, pic_fea], dim=1)
        fusion_fea = fusion_fea.unsqueeze_(dim=2).unsqueeze_(dim=2)
        fusion_fea = self.deepconv(fusion_fea)
        fusion_fea = fusion_fea.flatten(start_dim=1)
