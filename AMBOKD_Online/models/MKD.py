...
def segment_dataset(X, window_size, step):
    win_x = []
    # X = np.transpose(X, [0, 2, 1])  # [trial, time_length, channel]
...
class gcn(nn.Module):
    def __init__(self, num_features=30, num_classes=2):
        ...
        # self.gcn2 = GCNConv(85, 60)
        # self.gcn3 = GCNConv(85, 60)
        #self.gcn2 = GCNConv(60,20)
    ...
    def forward(self, data):
        # x, edge_index, batch = data.x, data.edge_index, data.batch#x[12977,90],edge_index[2,66230],batch(12977)
       ...
       ##slice
        ...
        x = x.reshape(batch_size,59,window,-1)
        x= x.permute(0,2,1,3)
        # x22 = self.gcn2(x, edge_index, weight).unsqueeze_(dim=2)
        # x33 = self.gcn3(x, edge_index, weight).unsqueeze_(dim=2)
...
class fusion(nn.Module):##利用attention实现融合,两模态concat，利用注意力得到两模态的重要性权重
    def __init__(self, num_classes=2):
        super(fusion, self).__init__()
        ##图像特征处理
        self.MCGRAM = gcn()
        self.efficientNet = efficient()
        self.pic_fc = nn.Linear(1280,64)
        self.eeg_fc = nn.Linear(256, 64)
        self.attention = selfAttention(num_attention_heads=2 , input_size=64, hidden_size=64)
    ...
    def forward(self, data):
        ...
        pic_fea = self.pic_fc(pic_fea)
        eeg_fea = self.eeg_fc(eeg_fea)
        fusion_fea = torch.cat([eeg_fea.unsqueeze_(dim=1), pic_fea.unsqueeze_(dim=1)], dim=1)
        fusion_fea = self.attention(fusion_fea,fusion_fea)
        fusion_fea = fusion_fea.view(batch_size, -1)
        fusion_classes = self.fc_final(fusion_fea)
        # fusion_classes = torch.softmax(fusion_classes, dim=1)
        return fusion_classes##利用attention实现融合
